...
def single_exponential_smoothing(alpha, y_test):
    ...
    y_pred = ses_model1.predict(328).rename(r'$\alpha=%s$' % ses_model1.model.params['smoothing_level'])

    fig = plt.figure(figsize=(60, 8))
    y_pred[1:].plot(color='grey', legend=True)
    ...
    mse_ses = mean_squared_error(y_test[327:-1], y_pred)
